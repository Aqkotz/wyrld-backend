...
describe('resourceService', () => {
  ...
  describe('getResources', () => {
    it('Can get resource', async () => {
      const resource = await resourceService.getResources({ _id: idResourceA })
        .then((res) => res[0])
        .catch(() => undefined);
    ...
    it('Rejects if resource does not exist', async () => {
      const resource = await resourceService.getResources({ _id : invalidId })
        .then((res) => res[0])
        .catch(() => undefined);
  ...
  describe('updateResource', () => {
    ...
    it('Updates resource field, returns updated resource', async () => {
      ...
      const updatedResource2 = await resourceService.getResources({ _id: idResourceA })
        .then((res) => res[0])
        .catch(() => undefined);
