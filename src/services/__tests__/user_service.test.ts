...
const userDataA: Omit<IUser, '_id'> = {
...
const userDataB: Omit<IUser, '_id'> = {
...
const userDataC: Omit<IUser, '_id'> = {
...
describe('userService', () => {
  ...
  describe('getUsers', () => {
    it('Can get single user', async () => {
      const user = await userService.getUsers({ _id: idUserA })
        .then((res) => res[0])
        .catch(() => undefined);
      ...
      if (user !== undefined) { // For TypeScript purposes, need to check again
        ...
        expect(user._id.toString()).toStrictEqual(idUserA);
        expect(user.password).not.toBe(userDataA.password);
      }
    });

    it('Returns undefined if no users to get', async () => {
      const user = await userService.getUsers({ _id: invalidId })
        .then((res) => res[0])
        .catch(() => undefined);
  ...
  describe('updateUser', () => {
    it('Updates user field', async () => {
      ...
      const updatedUser2 = await userService.getUsers({ _id: idUserA })
        .then((res) => res[0])
        .catch(() => undefined);
  ...
  describe('deleteUser', () => {
    ...
    it('Deletes existing user B', async () => {
      ...
      const user = await userService.getUsers({ _id: idUserB })
        .then((res) => res[0])
        .catch(() => undefined);
      expect(user).toBe(undefined);
    });
